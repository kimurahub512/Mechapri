...
const ProductCard = ({ product, isMobile = false, haveAccount = false, rowCnt = 1, currentUserId = null }) => {
	...
	const handleClick = () => {
        ...
        const isFree = product.price === 0 || product.price === '無料';

        // Inspect current path for user-scoped shop context
        const pathParts = window.location.pathname.split('/');
        // Support both '/{userId}' and '/shop/{userId}' shapes
        let shopUserId = null;
        if (pathParts.length > 2 && pathParts[1] === 'shop' && /^\d+$/.test(pathParts[2])) {
            shopUserId = Number(pathParts[2]);
        } else if (pathParts.length > 1 && /^\d+$/.test(pathParts[1])) {
            shopUserId = Number(pathParts[1]);
        }
        const isOnUserShopPage = shopUserId !== null;

        // Determine ownership via multiple fallbacks
        const productOwnerId = product.user?.id || product.user_id || null;
        const isOwner = Boolean(
            currentUserId && (
                currentUserId === productOwnerId || (isOnUserShopPage && currentUserId === shopUserId)
            )
        );

        // If owner is viewing their own product, always go to ProductDetailsFree
        if (isOwner) {
            const detailsUrl = route('product.details', { id: product.id });
            console.log('detailsUrl', detailsUrl);
            router.visit(detailsUrl);
            return;
        }

        // For free products (when not owner), always go to ProductDetailsFree page
        if (isFree) {
            const detailsUrl = route('product.details', { id: product.id });
            router.visit(detailsUrl);
            return;
		...
		// If we're on a user's shop page (viewing another shop), use user-scoped routes
		if (isOnUserShopPage) {
		...
		return (
			<div 
				...
				<div className="flex flex-col items-start h-[228.905px] pb-[45.945px] self-stretch relative">
					{/* 2121111: Main Image */}
					<div onClick={handleClick} className={`flex h-[149.489px] max-w-[157.372px] px-[26.277px] py-[2.919px] flex-col justify-center items-center flex-shrink-0 self-stretch rounded-[5.839px] bg-[#F6F6F6] ${product.display_mode !== 'normal' ? 'overflow-hidden' : ''}`}>
	...
	return (
		<div 
			...
			<div className="flex flex-col items-start self-stretch relative">
				{/* 2121111: Main Image */}
								 <div onClick={handleClick} className={`flex h-[256px] max-w-[269.5px] px-[45px] py-[5px] flex-col justify-center items-center flex-shrink-0 self-stretch rounded-[10px] bg-[#F6F6F6] ${product.display_mode !== 'normal' ? 'overflow-hidden' : ''}`}>
					{product.display_mode === 'normal' ? (
						<img 
							src={product.image || '/default-product-image.svg'} 
							alt="product" 
				...
				{/* 2121112: Time duration badge */}
				<div className={`flex flex-row py-[2px] px-[8.363px] justify-center items-center gap-[2px] absolute left-[3.77px] top-[4px] rounded-[15px] ${product.badge1?.includes('ヶ月') ? 'bg-[#E862CB]' : 'bg-[#FF2AA1]'}`}>
							...
							<div className="flex w-[87.06px] h-[18px] pb-[1px] justify-end items-center gap-[6px]">
								<div className="flex w-[18px] h-[18px] p-[1.5px_0.5px_0.633px_0.5px] justify-center items-center aspect-square">
									<svg xmlns="http://www.w3.org/2000/svg" width="18" height="16" viewBox="0 0 18 16" fill="none">
										<g clipPath="url(#clip0_667_27039)">
